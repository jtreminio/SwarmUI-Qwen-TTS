...
type VoiceType = "custom" | "design" | "audio";
...
class QwenTTSUI {
    ...
    private registerAudioPromptPrefix(): void {
        promptTabComplete.registerPrefix(
            "audio",
            "Add a section of prompt text used as the dialogue script for Qwen-TTS.",
            () => [
    ...
    private init(): void {
        ...
        this.inputElem = document.getElementById("input_qwenttsvoices") as HTMLInputElement | null;
        if (!this.inputElem) {
        ...
        if (useInVideoInput) {
            let block: Element | null = useInVideoInput;
        ...
        if (!insertBeforeNode && document.getElementById("input_qwenttsmodel")) {
            const modelBlock = (document.getElementById("input_qwenttsmodel") as HTMLElement)?.closest?.(".input-group");
    ...
    private nextVoiceId(): string {
        this.voiceIdCounter += 1;
        return `${Date.now()}_${this.voiceIdCounter}`;
    }

    private loadVoicesFromInput(): void {
        this.voices = [];
        if (!this.inputElem) {
            return;
        }
    ...
    private normalizeVoice(raw: Partial<VoiceEntry>): VoiceEntry {
        const type = (raw.type as VoiceType) ?? "custom";
    ...
    private getVoiceTypeLabel(type: VoiceType): string {
        switch (type) {
    ...
    private isFeatureInstalled(): boolean {
    ...
    private buildVoiceSection(voice: VoiceEntry): HTMLElement {
    ...
    private buildAddButtons(): HTMLElement {
        const wrap = document.createElement("div");
    ...
    private addVoice(type: VoiceType): void {
        this.serializeVoicesFromUi();
        const voice: VoiceEntry = this.normalizeVoice({
            id: this.nextVoiceId(),
            type,
    ...
    private installChangeListener(): void {
        ...
        this.voiceList.addEventListener("click", (e) => {
            const btn = (e.target as Element).closest('button[data-qwentts-action="remove-voice"]');
            if (!btn) {
                return;
            }
            e.preventDefault();
            e.stopPropagation();
            const voiceWrap = (btn.closest("[data-qwentts-voice-id]") as HTMLElement | null);
        ...
        const handler = (e: Event) => {
            ...
            const wrap = target.closest("[data-qwentts-voice-id]") as HTMLElement | null;
    ...
    private handleFileInput(input: HTMLInputElement, voiceId: string): void {
    ...
    private updateVoiceFromUi(voiceId: string, voice: VoiceEntry): void {
    ...
    private serializeVoicesFromUi(): void {
    ...
    private scheduleVoiceSyncFromUi(): void {
    ...
    private isGroupEnabled(): boolean {
        const toggler = document.getElementById("input_group_content_qwentts_toggle") as HTMLInputElement | null;
        return !toggler || !!toggler.checked;
    }

    /** Validation runs only when the Qwen-TTS toggle is ON; when off, returns null (no error). */
    private validateVoices(): string | null {
    ...
    private tryWrapGenerate(): boolean {
    ...
    private startGenerateWrapRetry(intervalMs = 250): void {
